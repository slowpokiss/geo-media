...
;// CONCATENATED MODULE: ./src/js/AddForm.js
...
class AddForm {
  ...
  init() {
    this.popoverForm.querySelector(".popup-cancel").addEventListener("click", ev => {
      ev.preventDefault();
...
class Chat {
  ...
  init() {
    ...
    const addCallBack = ev => {
      ev.preventDefault();
      this.addFormClass.actual.forEach(id => this.addFormClass.popoverClass.closeElem(id));
      this.addFormClass.actual = [];
      if (this.addFormClass.checkFormValidity(this.addFormClass.popoverForm.elements)) {
        const geoInputValue = this.addFormClass.popoverForm.querySelector(".popup-input-value").value;
        if (this.audioPlayer) {
          this.sendAudio(this.audioPlayer, this.getCurrentDate(), geoInputValue);
        } else {
          this.sendMessage(this.formInput.value, this.getCurrentDate(), `[${geoInputValue}]`);
        }
    ...
    this.form.addEventListener("submit", async ev => {
      ...
      if (this.formInput.value.trim()) {
        ...
        this.sendMessage(this.formInput.value, this.getCurrentDate(), modifiedResponse);
    ...
    this.alertPopup.querySelector(".alert-cancel").addEventListener("click", ev => {
      ev.preventDefault();
      this.closeAlert();
    });
  }
  ...
  sendAudio(audio, time, coords) {
    ...
    messageTemplate.insertAdjacentHTML("beforeend", `
    <div class="message-data">
  ...
  startAudio() {
    ...
    (async () => {
      try {
        ...
        audioRecorder.start();
        this.sendButtons.querySelector(".apply-audio-btn").addEventListener("click", ev => {
          ev.preventDefault();
          audioRecorder.stop();
        ...
        this.sendButtons.querySelector(".cancel-audio-btn").addEventListener("click", ev => {
          ev.preventDefault();
          audioRecorder.stop();
          stream.getTracks().forEach(track => track.stop());
          this.sendButtons.removeChild(this.audioStats);
  ...
  showAudioStats() {
    ...
    this.audioStats.insertAdjacentHTML("beforeend", `
